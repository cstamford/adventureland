...
const { upgrade_impl } = require('../js/server_upgrade_mock.js');
var fs = require("fs");
var url = require("url");
...
function init_io() {
	io.on("connection", function (socket) {
		...
		socket.on("get_sim_state", function (data) {
			const player = players[socket.id];
			socket.emit("sim_state", {
				player: {
					computer: player.computer,
					esize: player.esize,
					gold: 6942069420694206942069420,
					items: new Array(player.items.length).fill(null),
					p: player.p,
					q: player.q,
					s: player.s,
					user: player.user,
				},
				B: { sell_dist: B.sell_dist },
			...
			if (player.q[name].ms <= 0) {
				...
				if (name == "upgrade") {
					...
					if (player.p.u_item && player.items[ref.num] && player.items[ref.num].name == "placeholder") {
						...
						player.items[ref.num] = { name: item.name, level: 0, oo: item.oo }
					}
...
function game_loop() {
	...
	setTimeout(game_loop, 0); // originally 24
}
